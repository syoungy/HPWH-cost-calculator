...
st.title("HPWH vs Gas Water Heater — Household Monthly Cost (v3.8.1)")
st.caption(
...
with st.sidebar:
    ...
    if analysis_mode == "All Michigan sample":
        period = st.selectbox(
            ...
            key="statewide_period",
        )

        # County-only values are not used in the statewide branch.
        county_choice = None
        county_electric_provider = None
        county_gas_provider = None
        county_electric_tariff = None
        county_gas_tariff = None
        county_period = None

    else:
        st.markdown("#### County scenario")
        ...
        county_provider_map = data.provider_map[
            data.provider_map["in.county_name"].astype(str)
            == county_choice
        ].copy()

        county_electric_providers = sorted(
            county_provider_map["elec_provd"]
            .dropna()
            .astype(str)
            .unique()
        )
        county_gas_providers = sorted(
            county_provider_map["gas_provd"]
            .dropna()
            .astype(str)
            .unique()
        )
        ...
        # Keep period defined for the shared calculation setup below.
        ...
        for tariff_code, tariff_group in electric_rate_subset.groupby(
            ...
            if county_period in {"1", "8"}:
                ...
                valid = any(
                    set(year_group["month"].astype(int))
                    >= set(range(1, 13))
        ...
        if not valid_electric_tariffs:
            st.error(
                f"No complete electricity tariff is available for "
                f"{county_choice} and {PERIOD_OPTIONS[county_period]}."
            )
            st.stop()
        ...
        gas_rate_subset = data.gas_rates[
            data.gas_rates["gas_provd"].astype(str)
            == county_gas_provider
        ].copy()
        ...
        if not valid_gas_tariffs:
            st.error(
                f"No gas tariff is available for {county_choice}."
            )
            st.stop()
        ...
        st.caption(
            f"Sample households: "
            f"{county_provider_map['bldg_id'].nunique()}"
        )
...
if analysis_mode == "All Michigan sample":
    ...
    **Chart display**
    ...
    st.caption(
        "Use the controls in the left sidebar to select County, electricity "
        "tariff, gas tariff, and period. This calculation keeps only sample "
        "households in the selected county and applies the selected tariffs "
        "to every included household."
    )
