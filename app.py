"""
HPWH vs Gas Water Heater — Household Range Calculator (v2.4.1)
...
def sort_in_source_order(df: pd.DataFrame) -> pd.DataFrame:
    """
    Sort by workbook order when that optional column exists.

    Older cached or previously deployed loaders may not provide
    ``source_order``. Pandas already preserves the existing row order, so
    returning the frame unchanged is the correct compatibility fallback.
    """
    if "source_order" in df.columns:
        return df.sort_values("source_order", kind="stable")
    return df
...
def latest_gas_row(
    ...
    latest = sort_in_source_order(subset[subset["year"] == latest_year])
...
for _, house in houses.iterrows():
    ...
    for tariff in provider_rates["tariff"].drop_duplicates():
        ...
        year = common_years[-1]
        gas_candidates = sort_in_source_order(
            data.gas_rates[
                (data.gas_rates["gas_provd"] == gas_provider)
...
for provider, group in data.gas_rates.groupby("gas_provd", sort=False):
    ...
    rows = sort_in_source_order(group[group["year"] == latest_year])
