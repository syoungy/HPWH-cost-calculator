...
def calculate_county_scenario(
    ...
    """Calculate one selected electric and gas tariff for one county sample."""
    if period not in {"1", "8", "year"}:
        raise ValueError(f"Unsupported period: {period}")

    county_houses = data.provider_map[
        data.provider_map["in.county_name"].astype(str) == str(county)
    ].copy()
    if county_houses.empty:
        raise ValueError(f"No mapped sample households found for {county}.")

    # Only retain houses mapped to the selected county providers. This keeps
    # the scenario grounded in actual provider coverage rather than applying
    # an unrelated utility rate to a house.
    county_houses = county_houses[
        (county_houses["elec_provd"].astype(str) == str(electric_provider))
        & (county_houses["gas_provd"].astype(str) == str(gas_provider))
    ].copy()
    if county_houses.empty:
        raise ValueError(
            f"No {county} households are jointly mapped to "
            f"{electric_provider} and {gas_provider}."
        )

    electric_years = _valid_electric_years(
        data.electricity_rates,
        electric_provider,
        electric_tariff,
        period,
    )
    if not electric_years:
        raise ValueError(
            f"No complete electricity-rate data for {electric_provider} / "
            f"{electric_tariff} / {period}."
        )
    electric_year = max(electric_years)

    gas_rows = data.gas_rates[
        (data.gas_rates["gas_provd"].astype(str) == str(gas_provider))
        & (data.gas_rates["tariff"].astype(str) == str(gas_tariff))
    ].copy()
    if gas_rows.empty:
        raise ValueError(
            f"No complete gas-rate data for {gas_provider} / {gas_tariff}."
        )
    gas_year = int(gas_rows["year"].max())
    gas_rows = gas_rows[gas_rows["year"] == gas_year].copy()
    if "source_order" in gas_rows.columns:
        gas_rows = gas_rows.sort_values("source_order", kind="stable")
    gas_row = gas_rows.iloc[0]
    gas_rate_vector = (
        gas_row[RATE_HOUR_COLUMNS].astype(float).to_numpy()
    )

    electric_usage = data.electricity_usage[
    ...
    # The merge suffixes all overlapping hourly usage columns.
    electric_columns = [
        f"{column}_electric" for column in USAGE_HOUR_COLUMNS
    ]
    gas_columns = [
        f"{column}_gas" for column in USAGE_HOUR_COLUMNS
    ]
    if houses[electric_columns + gas_columns].isna().any().any():
        raise ValueError(
            f"At least one {county} household is missing the selected "
            f"{period} usage profile."
        )

    records: list[dict] = []
    for _, house in houses.iterrows():
        electric_vector = house[electric_columns].astype(float).to_numpy()
        gas_vector = house[gas_columns].astype(float).to_numpy()

        electric_cost = _electric_monthly_cost(
            electric_vector,
            data.electricity_rates,
            electric_provider,
            electric_tariff,
        ...
        records.append({
            ...
            "electric_year": electric_year,
            "gas_provider": gas_provider,
            "gas_tariff": gas_tariff,
            "gas_year": gas_year,
            "hpwh_daily_use_kwh": float(electric_vector.sum()),
            "gas_wh_daily_use_kwh": float(gas_vector.sum()),
            "hpwh_monthly_cost": electric_cost,
            "gas_wh_monthly_cost": gas_cost,
            "hpwh_minus_gas": electric_cost - gas_cost,
        })

    household_results = pd.DataFrame(records)
    return CountyScenarioResult(
        households=household_results,
        electric_summary=_summary(household_results["hpwh_monthly_cost"]),
        gas_summary=_summary(household_results["gas_wh_monthly_cost"]),
        difference_summary=_summary(household_results["hpwh_minus_gas"]),
        electric_provider=electric_provider,
        gas_provider=gas_provider,
        electric_tariff=electric_tariff,
        gas_tariff=gas_tariff,
