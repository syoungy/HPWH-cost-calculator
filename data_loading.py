...
def _validate_space_heating_usage_pair(
    ...
    """Validate paired space-heating profiles while allowing summer zeros."""
    required_profiles = {"1", "8", "year"}
    electricity_ids = set(electricity_usage["bldg_id"].astype(str))
    gas_ids = set(gas_usage["bldg_id"].astype(str))
    ...
    for usage_name, usage_df in [
        ...
        # Zero August space-heating profiles are physically plausible and are
        # intentionally allowed. Every household must nevertheless have some
        # positive space-heating use across the three stored profiles.
        row_totals = hourly.sum(axis=1)
        household_totals = pd.DataFrame(
            {
                "bldg_id": usage_df["bldg_id"].astype(str),
                "profile_total": row_totals,
            }
        ).groupby("bldg_id")["profile_total"].sum()
        nonpositive_households = household_totals[household_totals <= 0]
        if not nonpositive_households.empty:
            raise ValueError(
                f"{sample_label} {usage_name} contains households with no "
                "positive space-heating use in January, August, or the annual "
                f"profile. Examples: {nonpositive_households.index[:10].tolist()}"
            )
...
def load_space_heating_data(data_dir: Path = DATA_DIR) -> CalculatorData:
    ...
    _validate_space_heating_usage_pair(
...
def load_calculator_data(data_dir: Path = DATA_DIR) -> CalculatorData:
    ...
    return CalculatorData(
        ...
        excluded_building_ids=county_data.excluded_building_ids,
    )
